...
- **`Groups`** — `Unit`, `GroupID`, `Day`, `Start`, `End`, `TutorIDs` — defines this unit's slots with clock meeting times (the old `TimeSlot` AM/PM is gone). `TutorIDs` (spec §2 writes `TutorID(s)`) holds a comma/semicolon/slash-separated list — `splitIds()` parses it for co-tutors. If a unit has **no** Groups rows, the solver **auto-builds one group per assigned tutor** (time-less).
